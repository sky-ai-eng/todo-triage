...
// TestWithIdentityTx_RoutesPerTriggerType pins the helper's branch:
// manual runs invoke manualFn under a tx, event-triggered runs
// invoke systemFn against the bare stores bundle. Each fn is
// exercised once per call — the other must not run.
func TestWithIdentityTx_RoutesPerTriggerType(t *testing.T) {
	stores, _ := newStores(t)
	ctx := context.Background()

	manualCalls := 0
	systemCalls := 0
	mFn := func(db.TxStores) error { manualCalls++; return nil }
	sFn := func(db.Stores) error { systemCalls++; return nil }

	// Manual identity (synth-claims tx wraps).
	manualIdent := RunIdentity{
		OrgID:            runmode.LocalDefaultOrg,
		UserID:           runmode.LocalDefaultUserID,
		RunID:            "m1",
		IsEventTriggered: false,
	}
	if err := WithIdentityTx(ctx, stores, manualIdent, mFn, sFn); err != nil {
		t.Fatalf("manual WithIdentityTx: %v", err)
	}
	if manualCalls != 1 || systemCalls != 0 {
		t.Errorf("manual: manualCalls=%d systemCalls=%d, want 1/0", manualCalls, systemCalls)
	}

	// Event-triggered identity (admin-pool, no tx).
	eventIdent := RunIdentity{
		OrgID:            runmode.LocalDefaultOrg,
		UserID:           "",
		RunID:            "e1",
		IsEventTriggered: true,
	}
	if err := WithIdentityTx(ctx, stores, eventIdent, mFn, sFn); err != nil {
		t.Fatalf("event WithIdentityTx: %v", err)
	}
	if manualCalls != 1 || systemCalls != 1 {
		t.Errorf("event: manualCalls=%d systemCalls=%d, want 1/1", manualCalls, systemCalls)
	}
}
