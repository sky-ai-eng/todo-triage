...
- **Schema: goose-managed forward migrations.** New migrations land as `internal/db/migrations-sqlite/NNNNNNNNNNNN_description.sql` (12-digit `YYYYMMDDNNNN` version) with `-- +goose Up` / `-- +goose Down` markers. Down blocks are `SELECT 'down not supported';` no-ops — we don't ship downgrades for installed user-tools. The runner (`internal/db/migrations.go`) classifies each boot into one of four install states (already-goose-managed / fresh / legacy-runner-populated / pre-runner) and stamps existing installs at the consolidated baseline; pre-runner installs (app tables but no version metadata) are refused with `ErrPreRunnerInstall` pointing the operator at v1.10.1 as the intermediate upgrade hop. See `docs/specs/sky-245-d1-goose-migrations.html`. The legacy `schema_migrations` table is left in place as an audit trail. `scripts/clean-slate.sh` is dev-only and not a production upgrade path. Postgres migrations live in `internal/db/migrations-postgres/` (multi-mode schema, SKY-247 / D3); `db.Migrate(db, dialect)` routes to the matching tree. Postgres tests use the `internal/db/pgtest` harness — testcontainer with two connections (AdminDB superuser, AppDB authenticator+tf_app) — and skip cleanly when Docker isn't available. See `docs/specs/sky-247-d3-multi-tenant-postgres-schema.html`.
