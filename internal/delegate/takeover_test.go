...
func newSpawnerWithActiveCancel(database *sql.DB, runID string) *Spawner {
	s := NewSpawner(database, testPromptStore(database), nil, nil, testTaskStore(database), sqlitestore.New(database).AgentRuns, sqlitestore.New(database).Entities, sqlitestore.New(database).Reviews, nil, nil, "claude-sonnet-4-6")
	if runID != "" {
...
func TestTakeover_EmptyBaseDir(t *testing.T) {
	s := NewSpawner(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, "")
...
func TestTakeover_NonexistentRun(t *testing.T) {
	...
	s := NewSpawner(database, testPromptStore(database), nil, nil, testTaskStore(database), sqlitestore.New(database).AgentRuns, sqlitestore.New(database).Entities, sqlitestore.New(database).Reviews, nil, nil, "")
