-- Baseline schema. Captured verbatim from the previous in-Go `schema`
-- const at the moment forward migrations were introduced. Existing
-- installs are stamped as already-applied (the runner detects a
-- populated DB and skips this file's exec); only fresh installs run
-- it. All future schema changes go in new NNN-numbered migrations
-- alongside this file — never edit this baseline.
--
