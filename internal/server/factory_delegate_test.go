...
func TestHandleFactoryDelegate_DelegateErrorPreservesClaim(t *testing.T) {
	s := newTestServer(t)
	s.SetSpawner(delegate.NewSpawner(s.db, s.prompts, nil, s.chains, s.tasks, s.agentRuns, s.entities, s.reviews, s.pendingPRs, s.events, s.taskMemory, nil, websocket.NewHub(), "haiku"))
