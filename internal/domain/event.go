...
// Event is a single occurrence of an EventType. In the new model it is keyed
// by entity_id (nullable for system events without entity context), not by
// task_id — task linkage moves to the `task_events` junction. TaskID is kept
// on the struct as a stale field for compile compat with downstream code that
// hasn't been rewritten yet.
type Event struct {
	ID        int64     `json:"id,omitempty"` // DB-assigned
	EventType string    `json:"event_type"`   // FK to events_catalog.id
	TaskID    string    `json:"task_id"`      // deprecated — use task_events junction
	EntityID  string    `json:"entity_id"`    // FK to entities.id (nullable for system events)
	...
	SourceID  string    `json:"source_id"`    // deprecated — identity lives on entities
